...
export function WeatherAccordion({ weather, temperatureUnit }: WeatherAccordionProps) {
  ...
  weather.history_data.forEach((day) => {
    // Fallback to current year if year is missing to prevent grouping errors
  ...
  const historicalYearStats = Array.from(yearGroups.entries())
    .map(([year, days]) => {
      // Use temp_max if available, otherwise fallback to max_temp_c, then avg_temp_c
      const avgTemp = days.length > 0
        ? Math.round(days.reduce((sum, d) => sum + (d.temp_max ?? d.max_temp_c ?? d.avg_temp_c ?? 0), 0) / days.length)
        : 0;
      const rainDays = days.filter((d) => (d.rain_sum ?? 0) > 1.0).length;
      ...
      const fourteenDayTrend = days.slice(0, 14).map(d => ({
        temp: d.temp_max ?? d.max_temp_c ?? d.avg_temp_c ?? 0,
        rain: d.rain_sum || 0,
        date: d.date,
      }));
                ...
                <div className="px-4 pb-4 pt-2 border-t border-foreground/10 animate-in slide-in-from-top-1 duration-200">
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-sm font-bold">{stat.humidity ? `${stat.humidity}%` : 'N/A'}</div>
                    </div>
                    <div className="space-y-1">
                      <div className="text-[10px] uppercase font-bold text-foreground/40">Sunset</div>
                      <div className="text-sm font-bold flex items-center gap-2"><Sunset className="h-3.5 w-3.5 text-orange-500" /> {stat.sunset || 'N/A'}</div>
                    </div>
                  </div>
